...
class shiftobjs:
	...
	def addobjs(self, common_objects, focpos, focalshift):
		...
		for f in common_objects:
			...
			self.objs.append(focobj(f['x'], f['y'], f['a'], f['b'], focpos, f['fwhm'], f['theta']))
...
def main():
	...
	for f in files[:]:
		...
		bkg_rms = bkg.rms()
		data_sub = data - bkg
		objects = sep.extract(data_sub, 4.5, err=bkg.globalrms, minarea=30)
		print len(objects)
		shifts = []
		fwhm_av = []
		...
		for o in objects:
			...
			if abs(ox - 275) > 1:
				...
				if len(common_objects) > 6 and len(common_objects) < 10:
