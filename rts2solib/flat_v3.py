...
__doc__ = """
    ...
    # installed. Please see median.py in RTS2/scripts for more details.
    #
    # As with all scripts intended to be called by RTS2 exe script command, you can
    # test this script by calling it and verifying that it prints something what
...
class Flat(object):
    ...
    filters are filter (single string) or filters (FILTA, FILTB.. array) for flats to expose
    """

    def __init__(self, filters, binning=None, ngood=None, window=None, expTimes=None):
...
# class: FlatScript()
# -
# noinspection PyDefaultArgument,PyDefaultArgument,PyPep8Naming,PyPackageRequirements,PyPackageRequirements
class FlatScript(scriptcomm.Rts2Comm):
    ...
    :param morningFlats: flats for morning. An arra of Flat class.
        If it is None, reversed :param:`eveningFlats` is used
    ...
    def __init__(self, eveningFlats=[Flat(None)], morningFlats=None,
                 maxBias=0, maxDarks=0, expTimes=list(range(1, 20))):
        ...
        # not met flat requirements. This is particularly useful for
        # debugging.
        ...
        self.exptime = None
        self.numberFlats = None
        self.usedFlats = None
        self.optimalFlat = None
        self.optimalRange = None
        self.allowedOptimalDeviation = None
        self.biasLevel = None
        self.defaultNumberFlats = None
        self.sleepTime = None

        self.eveningMultiply = None
        self.morningMultiply = None

        self.shiftRa = None
        self.shiftDec = None

    def flatLevels(self, optimalFlat=65536 / 3, optimalRange=0.3, allowedOptimalDeviation=0,
    ...
    def optimalExpTime(self, ratio, expMulti):
        ...
        # array.  This measurement is intended to set skyflat at
        # "standardize" exposure times, so only limited number of dark
    ...
    def acquireImage(self):
        ...
        to flats, 1 if it was too bright, -1 if it was too dark."""
 ...
 Please adjust biasLevel in flats script.'''.format(
        ...
        if self.expTimes[-1] < self.exptime > self.expTimes[0]:
            if self.isSubWindow:
                self.fullWindow()
 ...
 exptime {4} ret {5}'''.format(
    ...
    def takeDarks(self, usedConfigs):
        ...
        self.setValue('SHUTTER', 'DARK')
        self.log('I', f'usedConfigs={usedConfigs}')
 ...
 median: {5}'''.format(
    ...
    def getData(self, domeDevice='DOME', tmpDirectory='/tmp/'):
        ...
        self.log('I', f'tmpDirectory={tmpDirectory}')
        self.setValue('SHUTTER', 'LIGHT')
...
# Bad flats: {2}\n\n'''.format(datetime.today(), ';'.join(
#                    list(map(Flat.signature, self.goodFlats))),
#                ";".join(list(map(Flat.signature, self.badFlats)))
#            )
